...
class PendulumCartEnv(gym.Env):
    ...
    def step(self, action):
        ...
        # 3. Integración de Euler Semi-Implícito (Crucial para péndulos de 5cm)
        # PRIMERO actualizamos las velocidades con las aceleraciones actuales
        x_dot = x_dot + config.DT * x_acc
        theta_dot = theta_dot + config.DT * theta_acc
        
        # LUEGO actualizamos las posiciones usando las NUEVAS velocidades
        x = x + config.DT * x_dot
        ...
        if not terminated:
            # 1. Objetivo principal: Estar arriba. 
            # Da 1.0 en la vertical perfecta, 0.0 colgando hacia abajo.
            reward_theta = (1.0 + math.cos(theta)) / 2.0
            
            # 2. Recompensa de centrado multiplicada por el ángulo.
            # Solo gana puntos por centrarse si el péndulo está mirando hacia arriba.
            reward_x = math.exp(-((x / 0.15)**2))
            
            # 3. PENALIZACIONES POR IMPULSIVIDAD (La cura a la locura)
            # Castigo fuerte por usar los motores a tope. Obliga a buscar movimientos suaves.
            penalty_u = 0.1 * (voltage / config.MAX_VOLTAGE)**2
            
            # Castigo ligero por velocidades angulares extremas para evitar que lo use de ventilador.
            penalty_spin = 0.001 * (theta_dot)**2
            
            # Castigo por ir a la velocidad de la luz de un lado a otro.
            penalty_speed = 0.01 * (x_dot)**2
            
            # Suma total (quitamos el swing_bonus defectuoso)
            reward = reward_theta + (reward_theta * reward_x) - penalty_u - penalty_spin - penalty_speed
        else:
            # Castigo base por salirse de la vía
            reward = -5.0 
